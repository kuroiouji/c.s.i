...
export class ApiService {
    ...
    public callApiController(url: string, option: any = null): Promise<Object> {
        ...
        if (type == "POST") {
            ...
            return this.http.post(SERVER.PATH + url, _data, options)
                .toPromise()
                .then((res: Response) => {
                    if (showLoading) {
            ...
            return this.http.get(SERVER.PATH + url, options)
                .toPromise()
                .then((res: Response) => {
                    if (showLoading) {
                        this.spinner.hide(500);
    ...
    public downloadFile(url: string, option: any): Promise<Object> {
        return this.callApiController(url, option)
            .then(
                (result) => {
                    if (result != undefined) {
                        var wind = window.open(SERVER.PATH + "api/Common/Download?f=" + result, "_blank");
                        if (wind != undefined) {
                            wind.focus();
                        }
