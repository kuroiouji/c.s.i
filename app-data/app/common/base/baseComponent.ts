...
import { RoleData,PjRoleData }                     from "../guards";
import { ROLE_PERMISSION,ROLE_PROJECT }              from "../common.permission";
...
export abstract class ABaseComponent implements OnInit, AfterViewInit {
    ...
    public hasPermissionISSCancel(){
        return this.hasPermission(undefined,ROLE_PROJECT.CANCEL);
    }
    
    public hasPermissionISSCheck(){
        return this.hasPermission(undefined,ROLE_PROJECT.CHECK);
    }

    public hasPermissionISSClose(){
        return this.hasPermission(undefined,ROLE_PROJECT.CLOSE);
    }

    public hasPermissionISSEdit(){
        return this.hasPermission(undefined,ROLE_PROJECT.EDIT_ISSUE);
    }

    public hasPermissionISSMiss(){
        return this.hasPermission(undefined,ROLE_PROJECT.MISS);
    }
    
    public hasPermissionISSPlan(){
        return this.hasPermission(undefined,ROLE_PROJECT.PLAN);
    }

    public hasPermissionISSProcess(){
        return this.hasPermission(undefined,ROLE_PROJECT.PROCESS_FIX);
    }
    
    public hasPermissionISSFix(){
        return this.hasPermission(undefined,ROLE_PROJECT.PROCESS_FIX);
    }
    
    public hasPermissionISSRedebug(){
        return this.hasPermission(undefined,ROLE_PROJECT.RE_DEBUG);
    }

    public hasPermissionPJEdit(){
        return this.hasPermission(undefined,ROLE_PROJECT.EDIT_PROJECT);
    }

    public hasPermissionCreatePro(){
        return this.hasPermission(undefined,ROLE_PROJECT.CREATE_PRO);
    }

    public hasPermissionDeletePro(){
        return this.hasPermission(undefined,ROLE_PROJECT.DELETE_PRO);
    }
    ...
    private hasPermission(s: string, p: any): boolean {
        let role = this.route.snapshot.data["role"] as RoleData;
        let ProjectRole = this.route.snapshot.data["ProjectRole"] as PjRoleData;
        if (role == undefined && ProjectRole == undefined) {
            return false;
        }

        if (s != undefined && role !=undefined ) {
        ...
        if (ProjectRole !=undefined ) {
            return ProjectRole.hasPermission(p);           
        }
